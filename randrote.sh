...
rm -f maplist.cfg				# Clear the old map rotation
shuf -n $1 mappool > randmaps	# Shuffle the maps!

for map in $(cat randmaps)
do
	if [ $i -eq $1 ]			# If we're on the last line of the maps file
	then						# then we need nextmap vstr set to 'm01'.
		if [ $skipped ]
		then
			echo set m0$i \"map $skipped\; set nextmap vstr m01\" >> maplist.cfg
		elif [ $strike -eq 3 ]
		then
			echo set m0$i \"map $must\; set nextmap vstr m01\" >> maplist.cfg
		else
			echo set m0$i \"map $map\; set nextmap vstr m01\" >> maplist.cfg
		fi
		echo vstr m01 >> maplist.cfg	# The required last line of a map rotation file
	else
		if [ $map = $must ]
		then
			...
			then
				echo set m0$i \"map $skipped\; set nextmap vstr m0"$(($i+1))"\" >> maplist.cfg
				unset skipped
			else
				# We found our map, go ahead and write it out naturally.
				echo set m0$i \"map $map\; set nextmap vstr m0"$(($i+1))"\" >> maplist.cfg
			...
			then
				...
				echo set m0$i \"map $must\; set nextmap vstr m0"$(($i+1))"\" >> maplist.cfg
